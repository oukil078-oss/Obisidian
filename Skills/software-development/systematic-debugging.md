...
source_path: "C:\Users\pc\AppData\Local\hermes\skills\software-development\systematic-debugging\SKILL.md"
vault_path: "Skills/software-development/systematic-debugging.md"
tags: ["software-development", "hermes-skill", "skill", "debugging", "troubleshooting", "problem-solving", "root-cause", "investigation"]
trigger_keywords: ["4-phase", "root", "cause", "debugging", "understand", "bugs", "before", "fixing"]
...
**When the user selects a remediation path, that choice supersedes the agent's preferred fix strategy.**

- If the user says **"roll back"**, **"use the old version"**, **"revert"**, or **"skip to phase N"** — execute that directive directly.
- Do **not** substitute an alternate fix (e.g., patching the new implementation) when the user has explicitly chosen a different path.
- After executing the user's chosen remediation, verify it works before moving on.

### Why this matters

Ignoring the user's remediation choice wastes time, creates new bugs, and signals that the agent is optimizing for its own agenda rather than the user's stated goal.
...
- **"User asked to roll back, but I'll just make the new approach work"** → STOP. Execute the rollback.
...
### Platform-specific process inspection (Windows)

On Windows, `ps aux` and `pgrep` are unavailable. Use these instead:

```powershell
# Find processes by command line substring
Get-CimInstance Win32_Process -Filter "CommandLine LIKE '%pattern%'" | Select-Object ProcessId,CommandLine
...
**Key pitfall:** Windows Bash/MSYS wrappers spawn child `node.exe` or `python.exe` processes with different PIDs than the shell. Always filter by `CommandLine LIKE '%pattern%'` rather than matching shell PIDs.

**Stuck dev server pattern:**
1. Find via `CommandLine LIKE '%next%'`
2. Kill all matching PIDs
3. Verify no listeners remain: `netstat -ano | findstr ':3000 '`
4. Restart from correct directory
